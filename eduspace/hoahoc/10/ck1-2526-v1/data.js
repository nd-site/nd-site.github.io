window.quizData = {
    "title": "Kiểm tra Cuối Học Kì I – Hóa học 10 (KNTT) – 2025-2026",
    "config": {
        "testDuration": 2700,
        "examLayout": {
            "multiple": {
                "count": 12,
                "pointsPerQ": 0.25
            },
            "truefalse": {
                "count": 4,
                "pointsPerQ": 1,
                "partialScoring": [
                    0.1,
                    0.25,
                    0.5,
                    1
                ]
            },
            "short": {
                "count": 6,
                "pointsPerQ": 0.5
            }
        }
    },
    "questions": [
        {
            "type": "multiple",
            "question": "Thành phần cấu tạo của nguyên tử bao gồm:",
            "options": [
                "Proton và neutron.",
                "Hạt nhân và lớp vỏ electron.",
                "Electron và neutron.",
                "Proton và lớp vỏ electron."
            ],
            "correct": 1,
            "explanation": "Nguyên tử gồm hạt nhân (trung tâm) và lớp vỏ electron (quay xung quanh)."
        },
        {
            "type": "multiple",
            "question": "Các hạt mang điện trong nguyên tử là:",
            "options": [
                "Proton và neutron.",
                "Proton và electron.",
                "Electron và neutron.",
                "Chỉ có electron."
            ],
            "correct": 1,
            "explanation": "Proton mang điện dương (+), electron mang điện âm (-), neutron không mang điện."
        },
        {
            "type": "multiple",
            "question": "Số hiệu nguyên tử (Z) cho biết điều gì?",
            "options": [
                "Số lượng hạt neutron trong hạt nhân.",
                "Số lượng hạt proton trong hạt nhân.",
                "Khối lượng của nguyên tử.",
                "Số lớp electron của nguyên tử."
            ],
            "correct": 1,
            "explanation": "Số hiệu nguyên tử Z = số proton = số electron (trong nguyên tử trung hòa)."
        },
        {
            "type": "multiple",
            "question": "Ký hiệu nguyên tử $^{23}_{11}Na$ cho biết nguyên tử Sodium có:",
            "options": [
                "11 proton, 23 neutron.",
                "11 proton, 12 neutron.",
                "23 proton, 11 neutron.",
                "11 neutron, 12 proton."
            ],
            "correct": 1,
            "explanation": "Z = 11 (proton), A = 23 (số khối). Số neutron N = A - Z = 23 - 11 = 12."
        },
        {
            "type": "multiple",
            "question": "Bảng tuần hoàn các nguyên tố hóa học hiện nay được sắp xếp theo chiều tăng dần của:",
            "options": [
                "Khối lượng nguyên tử.",
                "Bán kính nguyên tử.",
                "Điện tích hạt nhân nguyên tử.",
                "Độ âm điện."
            ],
            "correct": 2,
            "explanation": "Nguyên tắc sắp xếp cơ bản là theo chiều tăng dần của số hiệu nguyên tử (Z)."
        },
        {
            "type": "multiple",
            "question": "Các nguyên tố thuộc cùng một nhóm A trong bảng tuần hoàn có đặc điểm chung là:",
            "options": [
                "Có cùng số lớp electron.",
                "Có cùng số electron hóa trị.",
                "Có cùng khối lượng nguyên tử.",
                "Có cùng tính chất vật lý."
            ],
            "correct": 1,
            "explanation": "Số electron hóa trị quyết định tính chất hóa học tương tự nhau của các nguyên tố trong cùng nhóm."
        },
        {
            "type": "multiple",
            "question": "Liên kết hóa học được hình thành do sự dùng chung các cặp electron giữa hai nguyên tử là:",
            "options": [
                "Liên kết ion.",
                "Liên kết cộng hóa trị.",
                "Liên kết hydrogen.",
                "Liên kết kim loại."
            ],
            "correct": 1,
            "explanation": "Liên kết cộng hóa trị hình thành giữa các nguyên tử có độ âm điện gần nhau."
        },
        {
            "type": "multiple",
            "question": "Liên kết trong phân tử NaCl (muối ăn) thuộc loại liên kết nào?",
            "options": [
                "Liên kết cộng hóa trị không phân cực.",
                "Liên kết cộng hóa trị phân cực.",
                "Liên kết ion.",
                "Liên kết hydrogen."
            ],
            "correct": 2,
            "explanation": "NaCl hình thành do lực hút tĩnh điện giữa ion $Na^+$ và $Cl^-$ (hiệu độ âm điện lớn)."
        },
        {
            "type": "multiple",
            "question": "Trong phản ứng oxi hóa - khử, chất oxi hóa là chất:",
            "options": [
                "Nhường electron, số oxi hóa tăng.",
                "Nhận electron, số oxi hóa giảm.",
                "Nhường electron, số oxi hóa giảm.",
                "Nhận electron, số oxi hóa tăng."
            ],
            "correct": 1,
            "explanation": "Khẩu quyết: 'Khử cho - O nhận'. Chất oxi hóa nhận electron nên số oxi hóa giảm xuống."
        },
        {
            "type": "multiple",
            "question": "Số oxi hóa của nguyên tử Nitrogen (N) trong phân tử $NH_3$ là:",
            "options": [
                "+3.",
                "-3.",
                "0.",
                "+5."
            ],
            "correct": 1,
            "explanation": "H có số oxi hóa +1, gọi x là số oxi hóa của N: $x + 1 \\\\times 3 = 0 \\\\Rightarrow x = -3$."
        },
        {
            "type": "multiple",
            "question": "Phản ứng nào sau đây là phản ứng oxi hóa - khử?",
            "options": [
                "$NaOH + HCl \\\\rightarrow NaCl + H_2O$",
                "$CaCO_3 \\\\rightarrow CaO + CO_2$",
                "$Zn + CuSO_4 \\\\rightarrow ZnSO_4 + Cu$",
                "$CaO + H_2O \\\\rightarrow Ca(OH)_2$"
            ],
            "correct": 2,
            "explanation": "Trong phản ứng này, Zn thay đổi từ 0 lên +2, Cu từ +2 xuống 0."
        },
        {
            "type": "multiple",
            "question": "Cấu hình electron của nguyên tử Neon (Z=10) là:",
            "options": [
                "$1s^2 2s^2 2p^4$",
                "$1s^2 2s^2 2p^6$",
                "$1s^2 2s^2 2p^5$",
                "$1s^2 2s^2 3s^2$"
            ],
            "correct": 1,
            "explanation": "Tổng số electron là 10, phân bố đầy vào lớp 1 và lớp 2."
        },
        {
            "type": "truefalse",
            "question": "Xét các phát biểu sau về cấu tạo nguyên tử và bảng tuần hoàn:",
            "options": [
                "Nguyên tử có kích thước rất lớn so với hạt nhân của nó.",
                "Các đồng tử (đồng vị) của cùng một nguyên tố có số neutron khác nhau.",
                "Trong một chu kì, đi từ trái sang phải, bán kính nguyên tử có xu hướng tăng dần.",
                "Độ âm điện đặc trưng cho khả năng hút electron của nguyên tử khi hình thành liên kết hóa học."
            ],
            "correct": [
                true,
                true,
                false,
                true
            ],
            "explanation": "Trong một chu kì, đi từ trái sang phải, bán kính nguyên tử giảm dần do lực hút hạt nhân tăng mạnh.",
            "correctAnswers": [
                true,
                true,
                false,
                true
            ]
        },
        {
            "type": "truefalse",
            "question": "Về liên kết hóa học:",
            "options": [
                "Liên kết cộng hóa trị không phân cực thường hình thành giữa hai nguyên tử của cùng một nguyên tố phi kim.",
                "Phân tử nước ($H_2O$) có liên kết cộng hóa trị phân cực.",
                "Liên kết hydrogen mạnh hơn liên kết cộng hóa trị.",
                "Tương tác van der Waals làm tăng nhiệt độ nóng chảy và nhiệt độ sôi của các chất."
            ],
            "correct": [
                true,
                true,
                false,
                true
            ],
            "explanation": "Liên kết hydrogen là liên kết yếu, yếu hơn nhiều so với liên kết hóa học (ion, cộng hóa trị).",
            "correctAnswers": [
                true,
                true,
                false,
                true
            ]
        },
        {
            "type": "truefalse",
            "question": "Về phản ứng oxi hóa - khử:",
            "options": [
                "Trong mọi phản ứng hóa học đều có sự thay đổi số oxi hóa của các nguyên tố.",
                "Quá trình oxi hóa là quá trình nhường electron.",
                "Sự khử và sự oxi hóa luôn xảy ra đồng thời trong một phản ứng oxi hóa - khử.",
                "Chất khử là chất bị khử trong phản ứng."
            ],
            "correct": [
                false,
                true,
                true,
                false
            ],
            "explanation": "Phản ứng trao đổi không thay đổi số oxi hóa. Chất khử là chất bị OXI HÓA.",
            "correctAnswers": [
                false,
                true,
                true,
                false
            ]
        },
        {
            "type": "truefalse",
            "question": "Về cấu hình electron và tính chất nguyên tố:",
            "options": [
                "Nguyên tử có 8 electron ở lớp ngoài cùng (trừ He) đều rất bền vững (khí hiếm).",
                "Nguyên tố s và p thuộc các nhóm A trong bảng tuần hoàn.",
                "Lớp thứ M (n=3) có tối đa 18 electron.",
                "Cấu hình electron của ion $Na^+$ (Z=11) là $1s^2 2s^2 2p^6 3s^1$."
            ],
            "correct": [
                true,
                true,
                true,
                false
            ],
            "explanation": "Ion $Na^+$ đã nhường 1 electron ở lớp ngoài cùng, cấu hình đúng là $1s^2 2s^2 2p^6$.",
            "correctAnswers": [
                true,
                true,
                true,
                false
            ]
        },
        {
            "type": "short",
            "question": "Tìm số lượng electron tối đa trong phân lớp p.",
            "correct": "6",
            "explanation": "Phân lớp p có 3 orbital, chứa tối đa 6 electron."
        },
        {
            "type": "short",
            "question": "Một nguyên tử có cấu hình electron lớp ngoài cùng là $3s^2 3p^5$. Xác định số hiệu nguyên tử (Z) của nguyên tố này.",
            "correct": "17",
            "explanation": "$1s^2 2s^2 2p^6 3s^2 3p^5$. Tổng số e = 17 (Chlorine)."
        },
        {
            "type": "short",
            "question": "Xác định số oxi hóa của lưu huỳnh (S) trong phân tử $H_2SO_4$.",
            "correct": "+6",
            "explanation": "$(+1 \\\\times 2) + x + (-2 \\\\times 4) = 0 \\\\Rightarrow x = +6$."
        },
        {
            "type": "short",
            "question": "Tính khối lượng nguyên tử trung bình của nguyên tố X biết X có hai đồng vị: $^{63}X (75\\%)$ và $^{65}X (25\\%)$. (Nhập số thập phân)",
            "correct": "63,5",
            "explanation": "$M = (63 \\\\times 75 + 65 \\\\times 25) / 100 = 63,5$."
        },
        {
            "type": "short",
            "question": "Nguyên tố nào có độ âm điện lớn nhất trong bảng tuần hoàn? (Viết tên tiếng Việt)",
            "correct": "Fluorine",
            "explanation": "Fluorine (F) có độ âm điện 3.98."
        },
        {
            "type": "short",
            "question": "Cần bao nhiêu electron để một nguyên tử Oxygen (Z=8) đạt được cấu hình bền vững của khí hiếm gần nhất?",
            "correct": "2",
            "explanation": "$1s^2 2s^2 2p^4$. Cần nhận thêm 2e để thành $2p^6$ (Neon)."
        }
    ],
    "examInfo": {
        "subject": "Hóa học",
        "grade": "Lớp 10",
        "date": "Năm học 2025 - 2026",
        "examName": "Kiểm tra Cuối Học Kì I",
        "attempt": "Bản 1"
    }
};
