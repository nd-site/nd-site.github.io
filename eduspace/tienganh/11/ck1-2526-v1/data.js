window.quizData = {
    "title": "Kiểm tra Cuối Học Kì I – Tiếng Anh 11 (Global Success) – 2025-2026",
    "config": {
        "testDuration": 2700,
        "examLayout": {
            "multiple": {
                "count": 12,
                "pointsPerQ": 0.25
            },
            "truefalse": {
                "count": 4,
                "pointsPerQ": 1,
                "partialScoring": [
                    0.1,
                    0.25,
                    0.5,
                    1
                ]
            },
            "short": {
                "count": 6,
                "pointsPerQ": 0.5
            }
        }
    },
    "questions": [
        {
            "type": "multiple",
            "question": "Choose the word whose underlined part is pronounced differently from that of the others:",
            "options": [
                "h**ea**lthy",
                "br**ea**k",
                "br**ea**th",
                "h**ea**vy"
            ],
            "correct": 1,
            "explanation": "'Break' is pronounced /breɪk/ (diphthong), while the others have the short /e/ sound."
        },
        {
            "type": "multiple",
            "question": "Regular exercise and a balanced diet are essential for a long and ______ life.",
            "options": [
                "health",
                "healthy",
                "healthily",
                "healthiness"
            ],
            "correct": 1,
            "explanation": "Adjective 'healthy' modifies the noun 'life'."
        },
        {
            "type": "multiple",
            "question": "The ______ gap often leads to misunderstandings between parents and children.",
            "options": [
                "generation",
                "bridge",
                "space",
                "distance"
            ],
            "correct": 0,
            "explanation": "Generation gap: khoảng cách thế hệ."
        },
        {
            "type": "multiple",
            "question": "In the future, many people will live in ______ cities with smart technology.",
            "options": [
                "sustainable",
                "noisy",
                "crowded",
                "polluted"
            ],
            "correct": 0,
            "explanation": "Sustainable cities: các thành phố bền vững."
        },
        {
            "type": "multiple",
            "question": "Viet Nam has been an active ______ of ASEAN since 1995.",
            "options": [
                "member",
                "partner",
                "leader",
                "friend"
            ],
            "correct": 0,
            "explanation": "Member of ASEAN: thành viên của ASEAN."
        },
        {
            "type": "multiple",
            "question": "Global ______ is caused by the increase of greenhouse gases in the atmosphere.",
            "options": [
                "warming",
                "cooling",
                "change",
                "storm"
            ],
            "correct": 0,
            "explanation": "Global warming: sự nóng lên toàn cầu."
        },
        {
            "type": "multiple",
            "question": "Parents should listen to their children's opinions to ______ the generation gap.",
            "options": [
                "widen",
                "bridge",
                "make",
                "create"
            ],
            "correct": 1,
            "explanation": "Bridge the gap: rút ngắn/lấp đầy khoảng cách."
        },
        {
            "type": "multiple",
            "question": "If we continue to burn fossil fuels, the Earth's temperature ______ rise.",
            "options": [
                "will",
                "would",
                "shall",
                "may"
            ],
            "correct": 0,
            "explanation": "First conditional for real possibilities in the future."
        },
        {
            "type": "multiple",
            "question": "Smart cities use modern technology to improve the ______ of life of their citizens.",
            "options": [
                "quantity",
                "quality",
                "amount",
                "number"
            ],
            "correct": 1,
            "explanation": "Quality of life: chất lượng cuộc sống."
        },
        {
            "type": "multiple",
            "question": "The main ______ of ASEAN is to promote regional peace and stability.",
            "options": [
                "problem",
                "goal",
                "difficulty",
                "conflict"
            ],
            "correct": 1,
            "explanation": "Goal: mục tiêu."
        },
        {
            "type": "multiple",
            "question": "I have been interested in ______ about environmental issues since I was at school.",
            "options": [
                "learning",
                "to learn",
                "learn",
                "learned"
            ],
            "correct": 0,
            "explanation": "Interested in + V-ing."
        },
        {
            "type": "multiple",
            "question": "The increase in carbon dioxide emissions has ______ to global warming.",
            "options": [
                "led",
                "caused",
                "resulted",
                "made"
            ],
            "correct": 0,
            "explanation": "Lead to something: dẫn đến điều gì."
        },
        {
            "type": "truefalse",
            "question": "Read the text about Cities of the Future and decide whether the statements are True or False:",
            "options": [
                "Future cities will be highly dependent on non-renewable energy.",
                "Smart technology will help manage traffic and energy consumption.",
                "Green spaces like parks and gardens will be removed to build more houses.",
                "Sustainable cities aim to reduce the impact on the environment."
            ],
            "correct": [
                false,
                true,
                false,
                true
            ],
            "explanation": "Future cities focus on renewable energy and green spaces.",
            "correctAnswers": [
                false,
                true,
                false,
                true
            ]
        },
        {
            "type": "truefalse",
            "question": "About Generation Gap:",
            "options": [
                "The generation gap only exists in Asian countries.",
                "Different views on music, fashion, and lifestyle contribute to the gap.",
                "Open communication can help resolve conflicts between family members.",
                "Younger people always have the same values as their grandparents."
            ],
            "correct": [
                false,
                true,
                true,
                false
            ],
            "explanation": "The gap is a global issue. Generations often have different values.",
            "correctAnswers": [
                false,
                true,
                true,
                false
            ]
        },
        {
            "type": "truefalse",
            "question": "About ASEAN:",
            "options": [
                "ASEAN stands for Association of Southeast Asian Nations.",
                "The organization aims to promote economic growth in the region.",
                "There are 12 official member states of ASEAN.",
                "Viet Nam joined ASEAN before 1990."
            ],
            "correct": [
                true,
                true,
                false,
                false
            ],
            "explanation": "There are 10 (or 11 if including Timor-Leste officially) members. VN joined in 1995.",
            "correctAnswers": [
                true,
                true,
                false,
                false
            ]
        },
        {
            "type": "truefalse",
            "question": "About Healthy Lifestyle:",
            "options": [
                "Eating a lot of fast food is good for your long-term health.",
                "Getting enough sleep helps improve your mental health.",
                "Smoking is one of the leading causes of lung cancer.",
                "Stress has no effect on physical health."
            ],
            "correct": [
                false,
                true,
                true,
                false
            ],
            "explanation": "Fast food is unhealthy. Stress affects both mental and physical health.",
            "correctAnswers": [
                false,
                true,
                true,
                false
            ]
        },
        {
            "type": "short",
            "question": "Fill in the blank with the correct form of the word in brackets: 'The city is becoming more and more (SUSTAIN) ______.'",
            "correct": "sustainable",
            "explanation": "Sustainable: bền vững (adj)."
        },
        {
            "type": "short",
            "question": "Supply the correct form of the verb in brackets: 'They (WORK) ______ on this project for three months now.'",
            "correct": "have been working",
            "explanation": "Present perfect continuous for an action starting in the past and continuing."
        },
        {
            "type": "short",
            "question": "Identify the error and correct it: 'If I was you, I would take that job.' (Write the correct form to replace 'was')",
            "correct": "were",
            "explanation": "Second conditional: If + S + were..."
        },
        {
            "type": "short",
            "question": "Rewrite the sentence using 'Because of': 'Because the weather was bad, we stayed at home.' -> 'Because of ______, we stayed at home.'",
            "correct": "the bad weather",
            "explanation": "Because of + Noun phrase."
        },
        {
            "type": "short",
            "question": "What is the opposite of 'traditional' starting with 'm'? (6 letters)",
            "correct": "modern",
            "explanation": "Modern: hiện đại (trái nghĩa với truyền thống)."
        },
        {
            "type": "short",
            "question": "Fill in the blank with one suitable preposition: 'Viet Nam has made significant contributions ______ ASEAN.'",
            "correct": "to",
            "explanation": "Contribution to something: sự đóng góp vào cái gì."
        }
    ],
    "examInfo": {
        "subject": "Tiếng Anh",
        "grade": "Lớp 11",
        "date": "Năm học 2025 - 2026",
        "examName": "Kiểm tra Cuối Học Kì I",
        "attempt": "Bản 1"
    }
};
