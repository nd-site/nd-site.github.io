window.quizData = {
    "title": "Kiểm tra Cuối Học Kì I – Sinh học 12 (KNTT) – 2025-2026",
    "config": {
        "testDuration": 2700,
        "examLayout": {
            "multiple": {
                "count": 12,
                "pointsPerQ": 0.25
            },
            "truefalse": {
                "count": 4,
                "pointsPerQ": 1,
                "partialScoring": [
                    0.1,
                    0.25,
                    0.5,
                    1
                ]
            },
            "short": {
                "count": 6,
                "pointsPerQ": 0.5
            }
        }
    },
    "questions": [
        {
            "type": "multiple",
            "question": "Quá trình nhân đôi DNA diễn ra theo nguyên tắc nào?",
            "options": [
                "Nguyên tắc bổ sung và nguyên tắc bán bảo toàn.",
                "Nguyên tắc bổ sung và nguyên tắc tập trung.",
                "Nguyên tắc bán bảo toàn và nguyên tắc phân ly.",
                "Nguyên tắc bổ sung và nguyên tắc phân ly độc lập."
            ],
            "correct": 0,
            "explanation": "Nguyên tắc bổ sung (A-T, G-C) và bán bảo toàn (giữ lại một nửa mạch cũ)."
        },
        {
            "type": "multiple",
            "question": "Mã di truyền có tính đặc hiệu, nghĩa là:",
            "options": [
                "Nhiều bộ ba cùng mã hóa cho một loại amino acid.",
                "Một bộ ba chỉ mã hóa cho một loại amino acid.",
                "Tất cả các loài đều dùng chung một bộ mã di truyền.",
                "Mã di truyền được đọc từ một điểm xác định và không gối lên nhau."
            ],
            "correct": 1,
            "explanation": "Đây là đặc điểm giúp thông tin di truyền được truyền đạt chính xác."
        },
        {
            "type": "multiple",
            "question": "Dạng đột biến cấu trúc nhiễm sắc thể nào làm tăng số lượng gene trên một nhiễm sắc thể?",
            "options": [
                "Mất đoạn.",
                "Lặp đoạn.",
                "Đảo đoạn.",
                "Chuyển đoạn trên cùng một NST."
            ],
            "correct": 1,
            "explanation": "Lặp đoạn dẫn đến một đoạn NST được lặp lại một hay nhiều lần."
        },
        {
            "type": "multiple",
            "question": "Theo Mendel, quy luật phân ly độc lập thực chất nói về sự phân ly độc lập của:",
            "options": [
                "Các tính trạng.",
                "Các kiểu hình.",
                "Các cặp allele trong quá trình giảm phân tạo giao tử.",
                "Các dòng thuần chủng."
            ],
            "correct": 2,
            "explanation": "Sự phân ly của cặp allele này không phụ thuộc vào sự phân ly của cặp allele khác."
        },
        {
            "type": "multiple",
            "question": "Hiện tượng một gene tác động đến sự biểu hiện của nhiều tính trạng khác nhau được gọi là:",
            "options": [
                "Tương tác gene bổ trợ.",
                "Tương tác gene cộng gộp.",
                "Gene đa hiệu.",
                "Liên kết gene."
            ],
            "correct": 2,
            "explanation": "Ví dụ: Gene đột biến gây bệnh thiếu máu hồng cầu hình liềm ở người ảnh hưởng đến nhiều cơ quan."
        },
        {
            "type": "multiple",
            "question": "Ở quần thể tự phối (như đậu Hà Lan), qua nhiều thế hệ thành phần kiểu gene sẽ thay đổi theo hướng:",
            "options": [
                "Tăng dần tỉ lệ thể dị hợp, giảm dần tỉ lệ thể đồng hợp.",
                "Tăng dần tỉ lệ thể đồng hợp, giảm dần tỉ lệ thể dị hợp.",
                "Giữ nguyên tỉ lệ các kiểu gene.",
                "Tất cả các cá thể đều trở thành thể dị hợp."
            ],
            "correct": 1,
            "explanation": "Tự phối làm nghèo vốn gene, làm bộc lộ các tính trạng lặn có hại."
        },
        {
            "type": "multiple",
            "question": "Kỹ thuật nào sau đây được dùng để tạo sinh vật biến đổi gene (GMO)?",
            "options": [
                "Lai hữu tính.",
                "Kỹ thuật chuyển gene.",
                "Gây đột biến nhân tạo.",
                "Nuôi cấy mô tế bào."
            ],
            "correct": 1,
            "explanation": "Kỹ thuật chuyển gene cho phép đưa gene từ loài này sang loài khác một cách chủ động."
        },
        {
            "type": "multiple",
            "question": "Theo Darwin, nhân tố chính đóng vai trò chủ đạo trong quá trình tiến hóa là:",
            "options": [
                "Đột biến.",
                "Biến dị cá thể.",
                "Chọn lọc tự nhiên.",
                "Các cơ chế cách ly."
            ],
            "correct": 2,
            "explanation": "CLTN tích lũy các biến dị có lợi và đào thải các biến dị có hại cho sinh vật."
        },
        {
            "type": "multiple",
            "question": "Cơ quan tương đồng là những cơ quan:",
            "options": [
                "Có cùng nguồn gốc nhưng khác nhau về chức năng.",
                "Có cùng chức năng nhưng khác nhau về nguồn gốc.",
                "Chỉ xuất hiện ở các loài có quan hệ họ hàng rất xa.",
                "Hoàn toàn giống nhau về cấu tạo và chức năng."
            ],
            "correct": 0,
            "explanation": "Ví dụ: Cánh dơi và tay người (cùng là chi trước của thú)."
        },
        {
            "type": "multiple",
            "question": "Học thuyết tiến hóa tổng hợp hiện đại xác định nguồn biến dị sơ cấp cho tiến hóa là:",
            "options": [
                "Biến dị tổ hợp.",
                "Đột biến gene.",
                "Thường biến.",
                "Sự di nhập gene."
            ],
            "correct": 1,
            "explanation": "Đột biến tạo ra các allele mới, là nguyên liệu sơ cấp của tiến hóa."
        },
        {
            "type": "multiple",
            "question": "Bệnh nào sau đây ở người do đột biến số lượng nhiễm sắc thể gây ra?",
            "options": [
                "Mù màu.",
                "Máu khó đông.",
                "Hội chứng Down.",
                "Bạch tạng."
            ],
            "correct": 2,
            "explanation": "Hội chứng Down do thừa 1 chiếc NST ở cặp số 21 (Thể tam nhiễm 21)."
        },
        {
            "type": "multiple",
            "question": "Nhân tố tiến hóa nào có thể làm thay đổi tần số allele của quần thể một cách đột ngột và không theo một hướng xác định?",
            "options": [
                "Chọn lọc tự nhiên.",
                "Đột biến.",
                "Các yếu tố ngẫu nhiên.",
                "Giao phối không ngẫu nhiên."
            ],
            "correct": 2,
            "explanation": "Các yếu tố ngẫu nhiên (thiên tai, dịch bệnh) có thể loại bỏ bất kỳ cá thể nào khỏi quần thể."
        },
        {
            "type": "truefalse",
            "question": "Xét các phát biểu sau về cơ chế di truyền ở cấp độ phân tử:",
            "options": [
                "Trong quá trình phiên mã, mạch khuôn của gene được dùng để tổng hợp mRNA là mạch có chiều 3' -> 5'.",
                "Quá trình dịch mã kết thúc khi ribosome tiếp xúc với bộ ba kết thúc (UAA, UAG, UGA).",
                "Ở sinh vật nhân sơ, quá trình phiên mã và dịch mã diễn ra đồng thời trong nhân tế bào.",
                "Operon Lac ở vi khuẩn E.coli chỉ hoạt động khi môi trường có lactose."
            ],
            "correct": [
                true,
                true,
                false,
                true
            ],
            "explanation": "Sinh vật nhân sơ CHƯA có nhân hoàn chỉnh, các quá trình này diễn ra trong tế bào chất.",
            "correctAnswers": [
                true,
                true,
                false,
                true
            ]
        },
        {
            "type": "truefalse",
            "question": "Về quy luật di truyền của Mendel và Morgan:",
            "options": [
                "Liên kết gene hoàn toàn làm hạn chế sự xuất hiện của biến dị tổ hợp.",
                "Hoán vị gene xảy ra do sự tiếp hợp và trao đổi đoạn giữa 2 chromatid cùng nguồn gốc ở kì đầu giảm phân I.",
                "Tần số hoán vị gene không vượt quá 50%.",
                "Gene nằm trên NST giới tính X có hiện tượng di truyền chéo."
            ],
            "correct": [
                true,
                false,
                true,
                true
            ],
            "explanation": "Hoán vị gene xảy ra giữa 2 chromatid KHÁC nguồn gốc (không cùng chị em).",
            "correctAnswers": [
                true,
                false,
                true,
                true
            ]
        },
        {
            "type": "truefalse",
            "question": "Về di truyền quần thể và tiến hóa:",
            "options": [
                "Một quần thể đang ở trạng thái cân bằng Hardy-Weinberg thì tần số allele không thay đổi qua các thế hệ.",
                "Giao phối không ngẫu nhiên làm thay đổi tần số kiểu gene nhưng không làm thay đổi tần số allele.",
                "Chọn lọc tự nhiên tác động trực tiếp lên kiểu gene, từ đó làm thay đổi kiểu hình của sinh vật.",
                "Cách ly địa lý là nhân tố trực tiếp làm thay đổi tần số allele của quần thể."
            ],
            "correct": [
                true,
                true,
                false,
                false
            ],
            "explanation": "CLTN tác động trực tiếp lên KIỂU HÌNH. Cách ly địa lý chỉ ngăn cản sự giao phối, tạo điều kiện cho các nhân tố tiến hóa khác làm thay đổi tần số allele.",
            "correctAnswers": [
                true,
                true,
                false,
                false
            ]
        },
        {
            "type": "truefalse",
            "question": "Về bằng chứng và học thuyết tiến hóa:",
            "options": [
                "Bằng chứng tế bào học và sinh học phân tử là bằng chứng tiến hóa gián tiếp khách quan nhất.",
                "Tiến hóa nhỏ là quá trình biến đổi cấu trúc di truyền của quần thể dẫn đến hình thành loài mới.",
                "Chọn lọc chống lại allele lặn làm thay đổi tần số allele nhanh hơn chọn lọc chống lại allele trội.",
                "Đảo đại dương có hệ động thực vật nghèo nàn hơn đảo lục địa là bằng chứng địa lí sinh học."
            ],
            "correct": [
                true,
                true,
                false,
                true
            ],
            "explanation": "Chọn lọc chống lại allele TRỘI nhanh hơn vì allele trội luôn biểu hiện ra kiểu hình.",
            "correctAnswers": [
                true,
                true,
                false,
                true
            ]
        },
        {
            "type": "short",
            "question": "Một gene có 1200 nucleotide. Tính chiều dài ($\\\\mathring{A}$) của gene đó.",
            "correct": "2040",
            "explanation": "$L = (N/2) \\\\times 3,4 = 600 \\\\times 3,4 = 2040 \\\\mathring{A}$."
        },
        {
            "type": "short",
            "question": "Cho phép lai $AaBb \\\\times AaBb$. Tỉ lệ kiểu hình mang 2 tính trạng trội ở đời con là bao nhiêu? (Dạng phân số, ví dụ: 9/16)",
            "correct": "9/16",
            "explanation": "3/4 (A-) $\\\\times$ 3/4 (B-) = 9/16."
        },
        {
            "type": "short",
            "question": "Một quần thể có cấu trúc kiểu gene: $0,4 AA : 0,4 Aa : 0,2 aa$. Tính tần số allele A.",
            "correct": "0,6",
            "explanation": "$p(A) = 0,4 + 0,4/2 = 0,6$."
        },
        {
            "type": "short",
            "question": "Tên loại đột biến làm thay đổi vị trí của gene trên cùng một nhiễm sắc thể nhưng không làm thay đổi số lượng gene.",
            "correct": "Dao doan",
            "explanation": "Đột biến đảo đoạn NST."
        },
        {
            "type": "short",
            "question": "Khoảng cách giữa 2 gene A và B là 20 cM. Tính tần số hoán vị gene (%) giữa 2 gene này.",
            "correct": "20",
            "explanation": "Khoảng cách tính theo đơn vị centiMorgan chính bằng tần số hoán vị gene (%)."
        },
        {
            "type": "short",
            "question": "Xác định số lượng nhiễm sắc thể trong tế bào sinh dưỡng của người bị hội chứng Klinefelter (XXY).",
            "correct": "47",
            "explanation": "Bộ NST bình thường là 46, người này thừa 1 NST giới tính X."
        }
    ],
    "examInfo": {
        "subject": "Sinh học",
        "grade": "Lớp 12",
        "date": "Năm học 2025 - 2026",
        "examName": "Kiểm tra Cuối Học Kì I",
        "attempt": "Bản 1"
    }
};
