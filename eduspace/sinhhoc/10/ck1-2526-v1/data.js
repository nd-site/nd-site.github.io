window.quizData = {
    "title": "Kiểm tra Cuối Học Kì I – Sinh học 10 (KNTT) – 2025-2026",
    "config": {
        "testDuration": 2700,
        "examLayout": {
            "multiple": {
                "count": 12,
                "pointsPerQ": 0.25
            },
            "truefalse": {
                "count": 4,
                "pointsPerQ": 1,
                "partialScoring": [
                    0.1,
                    0.25,
                    0.5,
                    1
                ]
            },
            "short": {
                "count": 6,
                "pointsPerQ": 0.5
            }
        }
    },
    "questions": [
        {
            "type": "multiple",
            "question": "Các nguyên tố hóa học chính cấu tạo nên tế bào bao gồm:",
            "options": [
                "C, H, O, N.",
                "Ca, P, K, S.",
                "Fe, Cu, Zn, Mg.",
                "C, H, O, P."
            ],
            "correct": 0,
            "explanation": "C, H, O, N chiếm khoảng 96% khối lượng cơ thể sinh vật."
        },
        {
            "type": "multiple",
            "question": "Nước có vai trò quan trọng nhất đối với tế bào vì:",
            "options": [
                "Nước là thành phần cấu tạo chính của màng sinh chất.",
                "Nước là dung môi hòa tan nhiều chất, là môi trường cho các phản ứng sinh hóa.",
                "Nước cung cấp năng lượng cho tế bào.",
                "Nước giúp tế bào di chuyển dễ dàng."
            ],
            "correct": 1,
            "explanation": "Nước chiếm phần lớn khối lượng tế bào và tham gia vào hầu hết các hoạt động sống."
        },
        {
            "type": "multiple",
            "question": "Đơn phân cấu tạo nên phân tử Protein là:",
            "options": [
                "Nucleotide.",
                "Amino acid.",
                "Glucose.",
                "Fatty acid."
            ],
            "correct": 1,
            "explanation": "Có khoảng 20 loại amino acid chính cấu tạo nên các loại protein khác nhau."
        },
        {
            "type": "multiple",
            "question": "Chức năng chính của DNA là:",
            "options": [
                "Cấu tạo nên màng tế bào.",
                "Lưu trữ, bảo quản và truyền đạt thông tin di truyền.",
                "Xúc tác cho các phản ứng hóa học.",
                "Dự trữ năng lượng cho tế bào."
            ],
            "correct": 1,
            "explanation": "DNA (Acid Deoxyribonucleic) là vật chất di truyền ở cấp độ phân tử."
        },
        {
            "type": "multiple",
            "question": "Thành phần nào sau đây chỉ có ở tế bào nhân thực mà không có ở tế bào nhân sơ?",
            "options": [
                "Màng sinh chất.",
                "Ribosome.",
                "Nhân có màng bao bọc.",
                "Tế bào chất."
            ],
            "correct": 2,
            "explanation": "Tế bào nhân sơ chưa có nhân hoàn chỉnh (chỉ có vùng nhân)."
        },
        {
            "type": "multiple",
            "question": "Bào quan nào được coi là 'nhà máy năng lượng' của tế bào?",
            "options": [
                "Lưới nội chất.",
                "Bộ máy Golgi.",
                "Ti thể.",
                "Lục lạp."
            ],
            "correct": 2,
            "explanation": "Ti thể là nơi diễn ra quá trình hô hấp tế bào để tạo ra ATP (năng lượng)."
        },
        {
            "type": "multiple",
            "question": "Lục lạp là bào quan đặc trưng của loại tế bào nào?",
            "options": [
                "Tế bào động vật.",
                "Tế bào thực vật.",
                "Tế bào nấm.",
                "Tế bào vi khuẩn."
            ],
            "correct": 1,
            "explanation": "Lục lạp chứa diệp lục để thực hiện quá trình quang hợp ở cây xanh."
        },
        {
            "type": "multiple",
            "question": "Sự vận chuyển các chất từ nơi có nồng độ thấp đến nơi có nồng độ cao và cần tiêu tốn năng lượng được gọi là:",
            "options": [
                "Vận chuyển thụ động.",
                "Khuếch tán đơn giản.",
                "Vận chuyển chủ động.",
                "Thẩm thấu."
            ],
            "correct": 2,
            "explanation": "Vận chuyển chủ động (tích cực) đi ngược chiều gradient nồng độ nên cần năng lượng ATP."
        },
        {
            "type": "multiple",
            "question": "Enzyme là chất xúc tác sinh học có bản chất là:",
            "options": [
                "Lipid.",
                "Protein.",
                "Cacbonhydrat.",
                "Acid nucleic."
            ],
            "correct": 1,
            "explanation": "Hầu hết các enzyme đều là các phân tử protein có cấu trúc không gian đặc thù."
        },
        {
            "type": "multiple",
            "question": "Thành tế bào của thực vật có cấu tạo chính từ chất nào?",
            "options": [
                "Chitin.",
                "Peptidoglycan.",
                "Cellulose.",
                "Glycogen."
            ],
            "correct": 2,
            "explanation": "Cellulose tạo nên bộ khung bền vững cho tế bào thực vật."
        },
        {
            "type": "multiple",
            "question": "Thành phần chính của màng sinh chất theo mô hình khảm động là:",
            "options": [
                "Lớp kép phospholipid và protein.",
                "Lớp đơn lipid và carbohydrate.",
                "Sợi cellulose và pectin.",
                "Chỉ bao gồm các phân tử protein."
            ],
            "correct": 0,
            "explanation": "Khung là lớp kép phospholipid, xen kẽ là các phân tử protein (khảm) và có thể di chuyển (động)."
        },
        {
            "type": "multiple",
            "question": "Quá trình một tế bào thu nhận các phân tử lớn hoặc vi khuẩn vào bên trong bằng cách biến dạng màng sinh chất gọi là:",
            "options": [
                "Xuất bào.",
                "Thực bào.",
                "Khuếch tán.",
                "Thẩm thấu."
            ],
            "correct": 1,
            "explanation": "Thực bào là một hình thức của nhập bào, dùng để 'ăn' các chất rắn."
        },
        {
            "type": "truefalse",
            "question": "Xét các phát biểu sau về thành phần hóa học của tế bào:",
            "options": [
                "Các nguyên tố vi lượng (như Fe, I, Cu) chiếm tỉ lệ rất nhỏ nhưng cực kỳ quan trọng đối với sự sống.",
                "Cacbonhydrat (đường) là nguồn dự trữ năng lượng chính của tế bào.",
                "Protein chỉ đóng vai trò cấu trúc, không có chức năng bảo vệ hay điều hòa.",
                "Phân tử ATP được coi là 'đồng tiền năng lượng' của tế bào."
            ],
            "correct": [
                true,
                true,
                false,
                true
            ],
            "explanation": "Protein có rất nhiều chức năng: cấu trúc, xúc tác, bảo vệ, điều hòa, vận chuyển...",
            "correctAnswers": [
                true,
                true,
                false,
                true
            ]
        },
        {
            "type": "truefalse",
            "question": "Về cấu trúc tế bào nhân thực:",
            "options": [
                "Nhân tế bào là trung tâm điều khiển mọi hoạt động sống của tế bào.",
                "Ribosome là nơi tổng hợp lipid cho tế bào.",
                "Bộ máy Golgi có chức năng sửa đổi, đóng gói và phân phối sản phẩm của tế bào.",
                "Màng sinh chất có tính thấm chọn lọc, chỉ cho phép một số chất đi qua."
            ],
            "correct": [
                true,
                false,
                true,
                true
            ],
            "explanation": "Ribosome là nơi tổng hợp PROTEIN. Lưới nội chất trơn mới tổng hợp lipid.",
            "correctAnswers": [
                true,
                false,
                true,
                true
            ]
        },
        {
            "type": "truefalse",
            "question": "Về sự vận chuyển các chất qua màng:",
            "options": [
                "Khuếch tán đơn giản là sự di chuyển của các chất qua lớp kép phospholipid.",
                "Thẩm thấu là sự khuếch tán của các phân tử nước qua màng bán thấm.",
                "Vận chuyển thụ động luôn cần sự hỗ trợ của năng lượng ATP.",
                "Kênh protein giúp vận chuyển các chất có kích thước lớn hoặc tích điện qua màng."
            ],
            "correct": [
                true,
                true,
                false,
                true
            ],
            "explanation": "Vận chuyển thụ động KHÔNG tiêu tốn năng lượng ATP.",
            "correctAnswers": [
                true,
                true,
                false,
                true
            ]
        },
        {
            "type": "truefalse",
            "question": "Về Enzyme và chuyển hóa năng lượng:",
            "options": [
                "Mỗi loại enzyme thường chỉ xúc tác cho một vài phản ứng nhất định (tính đặc hiệu).",
                "Nhiệt độ quá cao có thể làm biến tính và mất hoạt tính của enzyme.",
                "Cơ chất là chất tham gia phản ứng được enzyme xúc tác.",
                "Enzyme giúp làm tăng năng lượng hoạt hóa của phản ứng hóa học."
            ],
            "correct": [
                true,
                true,
                true,
                false
            ],
            "explanation": "Enzyme làm GIẢM năng lượng hoạt hóa, từ đó làm tăng tốc độ phản ứng.",
            "correctAnswers": [
                true,
                true,
                true,
                false
            ]
        },
        {
            "type": "short",
            "question": "Viết tên loại đường đa (polysaccharide) dự trữ ở cơ thể động vật.",
            "correct": "Glycogen",
            "explanation": "Glycogen được dự trữ chủ yếu ở gan và cơ bắp."
        },
        {
            "type": "short",
            "question": "Phân tử mang thông tin di truyền từ DNA đến Ribosome để tổng hợp protein là loại RNA nào? (Viết tắt)",
            "correct": "mRNA",
            "explanation": "mRNA (messenger RNA) - RNA thông tin."
        },
        {
            "type": "short",
            "question": "Bào quan nào chứa các enzyme thủy phân dùng để tiêu hóa nội bào hoặc phân hủy các bào quan già? (1 từ)",
            "correct": "Lysosome",
            "explanation": "Lysosome được coi là 'nhà máy xử lý rác' của tế bào."
        },
        {
            "type": "short",
            "question": "Tính nồng độ chất tan (%) trong một tế bào nếu nồng độ dịch ngoại bào là 10% và tế bào đó không bị biến đổi hình dạng (trạng thái đẳng trương).",
            "correct": "10",
            "explanation": "Trạng thái đẳng trương xảy ra khi nồng độ chất tan bên trong và bên ngoài bằng nhau."
        },
        {
            "type": "short",
            "question": "Một phân tử DNA có 3000 nucleotide. Tính số lượng liên kết phosphodiester giữa các nucleotide trong phân tử đó.",
            "correct": "2998",
            "explanation": " DNA có 2 mạch, mỗi mạch n/2 nu. Số liên kết 1 mạch là (n/2 - 1). Tổng = 2 * (1500 - 1) = 2998."
        },
        {
            "type": "short",
            "question": "Viết đầy đủ tên của đơn vị năng lượng ATP.",
            "correct": "Adenosine triphosphate",
            "explanation": "Gồm bazơ nitơ Adenine, đường Ribose và 3 nhóm phosphate."
        }
    ],
    "examInfo": {
        "subject": "Sinh học",
        "grade": "Lớp 10",
        "date": "Năm học 2025 - 2026",
        "examName": "Kiểm tra Cuối Học Kì I",
        "attempt": "Bản 1"
    }
};
